...
try{const response=await fetch(`${origin}/api/jobs`,{method:'POST',headers:{Authorization:`Bearer ${keys.COLLECTOR_KEY}`},signal:AbortSignal.timeout(210000)});if(!response.ok)throw new Error(`HTTP ${response.status}`);const body=await response.json();report({ok:true,results:body.results});if(body.results?.every(r=>r.error||r.status==='error'))process.exitCode=1}catch(e){report({ok:false,error:e.message});process.exitCode=1}
